...
Day-to-day development creates caches, scratch files, local job logs, Terraform provider downloads, simulation shards, training CSVs, and model artifacts. The safe default cleanup removes only disposable caches and scratch files:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/cleanup_workspace.ps1
```

Larger generated outputs require explicit switches. Preview destructive cleanup first with `-WhatIf`:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/cleanup_workspace.ps1 -GeneratedData -WhatIf
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/cleanup_workspace.ps1 -GeneratedData
```

Useful switches:

- `-TerraformCache` removes `terraform/.terraform` provider downloads.
- `-WebJobs` removes dashboard job logs/state under `local/web_jobs`.
- `-GeneratedData` removes simulation shards, training CSVs, and profile metadata.
- `-ModelArtifacts` removes trained model outputs and experiment results.
- `-AllGenerated` combines the explicit generated-output cleanup switches.

## Notes

### Batch worker sizing
For initial tests, keep these small in Terraform:
- `job_vcpus = 1`
- `job_memory = 1024`

Then raise them later as needed.
...
Those are just safe starter values for your recent account constraints. Change them later when you move to larger paid Spot workers.

### No Python in the cloud workers
That is the whole point of this design:
- the cloud workers never need pandas, numpy, boto3, or your orchestration code
- they only need to:
  - download a chunk zip
  - run `simc`
  - upload a result zip

## Suggested next step

Once the basic flow works, the next improvement is usually adding:
- resumable runs for partially completed Batch specs
- child failure retry logic per chunk
- optional local manifest caching for interrupted orchestrator sessions

## Portfolio and repository hygiene

Before publishing this project to GitHub:

- initialize a Git repository from this folder and review `git status` before the first commit
- keep `.env`, `certs/`, `.aws/`, `terraform/terraform.tfstate*`, `terraform/terraform.tfvars`, generated training data, model artifacts, and temporary worker outputs out of Git
- use `terraform/terraform.tfvars.example` as the public configuration template
- set `AWS_ACCOUNT_ID` or `ECR_REPO` locally before running the ECR rebuild scripts
- rotate any certificate or AWS credential material that was ever copied into a public branch
